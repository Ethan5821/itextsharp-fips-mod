...
public class PdfEncryption {
    ...
    private byte[] ComputeOwnerKey(byte[] userPad, byte[] ownerPad) {
        ...
        byte[] digest = sha256.ComputeHash(ownerPad);
        if (revision == STANDARD_ENCRYPTION_128 || revision == AES_128) {
            byte[] mkey = new byte[keyLength / 8];
            // only use for the input as many bit as the key consists of
            for (int k = 0; k < 50; ++k)
                Array.Copy(sha256.ComputeHash(digest), 0, digest, 0, mkey.Length);
    ...
    private void SetupGlobalEncryptionKey(byte[] documentID, byte[] userPad, byte[] ownerKey, int permissions) {
        ...
        sha256.Initialize();
        sha256.TransformBlock(userPad, 0, userPad.Length, userPad, 0);
        ...
        sha256.TransformBlock(ext, 0, 4, ext, 0);
        if (documentID != null)
            sha256.TransformBlock(documentID, 0, documentID.Length, documentID, 0);
        if (!encryptMetadata)
            sha256.TransformBlock(metadataPad, 0, metadataPad.Length, metadataPad, 0);
        sha256.TransformFinalBlock(ext, 0, 0);

        byte[] digest = new byte[mkey.Length];
        Array.Copy(sha256.Hash, 0, digest, 0, mkey.Length);
    ...
    private void SetupUserKey() {
        if (revision == STANDARD_ENCRYPTION_128 || revision == AES_128) {
            sha256.TransformBlock(pad, 0, pad.Length, pad, 0);
            sha256.TransformFinalBlock(documentID, 0, documentID.Length);
    ...
    public void SetupAllKeys(byte[] userPassword, byte[] ownerPassword, int permissions) {
        if (ownerPassword == null || ownerPassword.Length == 0)
            ownerPassword = sha256.ComputeHash(CreateDocumentId());
        sha256.Initialize();
    ...
    public static byte[] CreateDocumentId() {
        ...
        return sha256.ComputeHash(Encoding.ASCII.GetBytes(s));
    }
    ...
    public void SetHashKey(int number, int generation) {
        sha256.Initialize();    //added by ujihara
        extra[0] = (byte)number;
        extra[1] = (byte)(number >> 8);
        extra[2] = (byte)(number >> 16);
        extra[3] = (byte)generation;
        extra[4] = (byte)(generation >> 8);
        sha256.TransformBlock(mkey, 0, mkey.Length, mkey, 0);
        sha256.TransformBlock(extra, 0, extra.Length, extra, 0);
        if (revision == AES_128)
            sha256.TransformBlock(salt, 0, salt.Length, salt, 0);
        sha256.TransformFinalBlock(extra, 0, 0);
        key = sha256.Hash;
        sha256.Initialize();
